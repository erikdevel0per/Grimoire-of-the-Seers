...
// Dados coletados na recepção do usuário
const userData = {
  name: "",
  reason: "",
  comments: ""
...
// Navegação do Formulário Inicial (Modal)
function nextStep(currentStep) {
  if (currentStep === 1) {
    const inputName = document.getElementById('inputName').value.trim();
    if (!inputName) return alert("Por favor, diga-nos seu nome.");
    userData.name = inputName;
    document.getElementById('step1').classList.remove('active');
    document.getElementById('step2').classList.add('active');
  } else if (currentStep === 2) {
    const inputReason = document.getElementById('inputReason').value.trim();
    if (!inputReason) return alert("Por favor, compartilhe o motivo da sua visita.");
    userData.reason = inputReason;
...
function finishIntro() {
  const inputComments = document.getElementById('inputComments').value.trim();
  userData.comments = inputComments;

  document.getElementById('introModal').style.display = 'none';

  const welcomeElement = document.getElementById('userWelcome');
  welcomeElement.textContent = `Consultando para: ${userData.name}`;
...
function generateSingleSynthesis(data) {
  tripleSynthesis.innerHTML = `
    <h3>Resultado do Oráculo para ${userData.name || 'Caminhante'}</h3>
...
function generateTripleSynthesis() {
  ...
  tripleSynthesis.innerHTML = `
    <h3>Convergência do Destino para ${userData.name || 'Caminhante'}</h3>
    <p>${synthesisText}</p>
  `;
  tripleSynthesis.style.display = 'block';
}
